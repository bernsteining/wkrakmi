...
wasm-bindgen ./target/wasm32-unknown-unknown/release/wkrakmi.wasm --out-dir build --no-typescript --no-modules --remove-name-section --remove-producers-section 

# optimize its size with wasm-opt from binaryen https://github.com/WebAssembly/binaryen
wasm-opt -Oz -o build/wkrakmi_bg.wasm build/wkrakmi_bg.wasm
